...
CONSTANTS LeaderToNetAggOrderRequest
...
MyInit == \* For MySwitchSpec: Starts with known roles
      LET r_switch  == switchIndex \* This will be "r1" from Toolbox
          r_leader  == CHOOSE s \in Servers : TRUE \* Pick one from {r2,r3,r4}
          r_netagg  == netAggIndex \* This will be "r5" from Toolbox (assuming r5 is added)
          r_followers == Servers \ {r_leader}
      IN
      /\ commitIndex = [s \in Server |-> 0]
      /\ currentTerm = [s \in Server |-> 2] \* Example term
      /\ entryCommitStats = << >>
      /\ leaderCount = [s \in Server |-> IF s = r_leader THEN 1 ELSE 0]
      /\ log = [s \in Server |-> <<>>]
      /\ matchIndex = [ s \in Server |-> [t \in Server |-> 0] ]
      /\ maxc = 0
      /\ messages = << >>
      /\ nextIndex = [ s \in Server |-> [t \in Server |-> 1] ]
      /\ state = [s \in Server |-> IF s = r_leader THEN Leader
                                   ELSE IF s = r_switch THEN Switch
                                   ELSE IF s = r_netagg THEN NetAgg
                                   ELSE Follower]
      /\ switchBuffer = [ v \in {} |-> [term |-> 0, value |-> "", payload |-> ""] ]
      /\ unorderedRequests = [ s \in Server |-> {} ]
      /\ switchSentRecord = [ s \in Server |-> {} ]
      /\ votedFor = [s \in Server |-> IF s = r_leader THEN Nil
                                      ELSE IF s \in Servers THEN r_leader
                                      ELSE Nil]
      /\ voterLog = [s \in Server |-> IF s = r_leader THEN [ot \in r_followers |-> <<>>]
                                      ELSE [ign \in {} |-> <<>>] ]
      /\ votesGranted = [s \in Server |-> IF s = r_leader THEN r_followers ELSE {}]
      /\ votesResponded = [s \in Server |-> IF s = r_leader THEN r_followers ELSE {}]
...
\* Action 3a: Leader delegates ordering of a request to NetAgg
LeaderDelegatesToNetAgg(ldr, na, val) ==
    /\ ldr \in Servers
    /\ state[ldr] = Leader
    /\ na = netAggIndex       \* Target is the NetAgg
    /\ val \in DOMAIN switchBuffer
    /\ val \in unorderedRequests[ldr] \* Leader should have the ID to delegate
    /\ LET reqToOrder == switchBuffer[val]
       IN Send([ mtype   |-> LeaderToNetAggOrderRequest,
                 mterm   |-> reqToOrder.term, \* Term from when request entered switchBuffer
                 mvalue  |-> val,             \* The ID of the request
                 msource |-> ldr,
                 mdest   |-> na ])
        /\ unorderedRequests' = [unorderedRequests EXCEPT ![ldr] = unorderedRequests[ldr] \ {val}] \* Leader has delegated it
        /\ UNCHANGED << serverVars, candidateVars, leaderVars, logVars, maxc, entryCommitStats, leaderCount,
                        switchBuffer, switchSentRecord >>

\* Action 3b: NetAgg receives the order delegation from the Leader
NetAggReceivesOrderDelegation(na, ldr, m) ==
    /\ na = netAggIndex
    /\ m.mdest = na
    /\ m.msource = ldr
    /\ ldr \in Servers
    /\ m.mtype = LeaderToNetAggOrderRequest
    \* TODO: NetAgg needs to store this task. For now, let's assume it just discards the message
    \* and in the next step, NetAgg will pick up 'm.mvalue' to replicate.
    \* For this step, we are just modeling the message flow.
    /\ Discard(m)
    \* If NetAgg had internal state to track requests, it would be updated here.
    \* For example: netAggTasks' = netAggTasks \union {[value |-> m.mvalue, term |-> m.mterm, status |-> "pending"]}
    /\ UNCHANGED << serverVars, candidateVars, leaderVars, logVars, instrumentationVars, hovercraftVars >>                          
...
MySwitchNext ==
   ...
   \/ \E raftSrv \in Servers, v \in DOMAIN switchBuffer :
       SwitchClientRequestReplicate(switchIndex, raftSrv, v)
   \* LeaderIngestHovercRaftRequest is replaced by the delegation actions below
   \/ \E ldr \in Servers, naNode \in {netAggIndex}, v \in DOMAIN switchBuffer : \* Leader Delegates
       state[ldr] = Leader /\ LeaderDelegatesToNetAgg(ldr, naNode, v)
   \/ \E naNode \in {netAggIndex}, ldrNode \in Servers, msg \in ValidMessage(messages): \* NetAgg Receives
       msg.mtype = LeaderToNetAggOrderRequest /\ msg.mdest = naNode /\ msg.msource = ldrNode /\
       NetAggReceivesOrderDelegation(naNode, ldrNode, msg)
   \* The following actions are for when NetAgg acts like a leader, or for standard Raft commit
   \/ \E srv \in Servers : AdvanceCommitIndex(srv)
   \/ \E srv1, srv2 \in Servers : srv1 /= srv2 /\ AppendEntries(srv1, srv2)
   \/ \E m \in {msg \in ValidMessage(messages) :
            msg.mdest \in Servers /\
...
\* Last modified Sat May 24 20:49:29 CEST 2025 by kusek
