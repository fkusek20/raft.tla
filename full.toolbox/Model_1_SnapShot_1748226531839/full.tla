...
MySwitchNext ==
   ...
   \/ \E srv1 \in Servers, srv2 \in Servers : \* srv1 is Leader, srv2 is Follower
       ...
       /\ state[srv1] = Leader
       /\ srv2 /= netAggIndex
       /\ LET leaderHasPendingMsgForNetAgg == \* Simpler check
              \E msg \in DOMAIN messages:
                  /\ msg.mtype = AppendEntriesRequest
                  /\ msg.msource = srv1
                  /\ msg.mdest = netAggIndex
                  /\ msg.mentries /= << >> \* It's a data-carrying AE, not just heartbeat
          IN /\ ( nextIndex[srv1][srv2] > Len(log[srv1]) \* Allow heartbeats if no entries to send
                  \/ ~leaderHasPendingMsgForNetAgg        \* OR if leader doesn't have a pending data AE for NetAgg
                )
