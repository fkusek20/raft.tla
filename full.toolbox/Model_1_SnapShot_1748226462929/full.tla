...
HandleAppendEntriesResponse(receiver, sender, m) ==
    ...
    /\ LET LeaderId == CHOOSE l \in Servers: state[l] = Leader
           currentLeaderMatchForSenderBeforeUpdate ==
               ...
               ELSE 0 \* Not applicable if NetAgg is receiver for this specific ack counting
          ...
          /\ \/ /\ m.msuccess
                /\ LET newMatchIndexForSender == Max({currentLeaderMatchForSenderBeforeUpdate, m.mmatchIndex})
                       entryKeyForAck == IF m.mmatchIndex > 0 /\ m.mmatchIndex <= Len(log[LeaderId])
                                           ...
                                           ELSE <<0,0>>
                       shouldIncrementAck ==
                           /\ receiver = LeaderId \* Only Leader updates its own ackCount directly
                           /\ entryKeyForAck /= <<0,0>>
                           ...
                           /\ currentLeaderMatchForSenderBeforeUpdate < entryKeyForAck[1]
...
MySwitchNext ==
   ...
   \/ \E srv1 \in Servers, srv2 \in Servers : \* srv1 is Leader, srv2 is Follower
       /\ srv1 /= srv2
       /\ state[srv1] = Leader
       /\ srv2 /= netAggIndex       \* Don't use this for direct AE to NetAgg (metadata goes via Ingress)
       /\ LET entryIndexToConsider == nextIndex[srv1][srv2]
              isForHovercRaftEntryJustSentToNetAgg ==
                  /\ entryIndexToConsider <= Len(log[srv1])
                  /\ \E msg_to_na \in DOMAIN messages: \* Check if there's a pending AE to NetAgg for this entry
                        /\ msg_to_na.mtype = AppendEntriesRequest
                        /\ msg_to_na.msource = srv1
                        /\ msg_to_na.mdest = netAggIndex
                        /\ msg_to_na.mentries /= << >>
                        /\ msg_to_na.mprevLogIndex + Len(msg_to_na.mentries) = entryIndexToConsider
                        /\ msg_to_na.mentries[1].value = log[srv1][entryIndexToConsider].value
          IN /\ ( entryIndexToConsider > Len(log[srv1]) \* Allow heartbeats
                  \/ ~isForHovercRaftEntryJustSentToNetAgg \* OR if not a HovercRaft entry pending for NetAgg
                )
